...
export default function Profile(){
  ...
  const [state, setState] = React.useState({ name: userDoc?.name || '', surname: userDoc?.surname || '', address: userDoc?.address || '', phone: userDoc?.phone || '', social: userDoc?.social || '' })
  const [saving, setSaving] = React.useState(false)
  const [msg, setMsg] = React.useState<string | null>(null)
  const addrRef = React.useRef<HTMLInputElement>(null); usePlacesAutocomplete(addrRef)
  React.useEffect(()=>{ setState({ name: userDoc?.name||'', surname: userDoc?.surname||'', address: userDoc?.address||'', phone: userDoc?.phone||'', social: userDoc?.social||'' }); if(addrRef.current) addrRef.current.value = userDoc?.address||'' },[userDoc])
  if (!user) return <div className='min-h-screen grid place-items-center text-white/70'>Please log in</div>
  const save = async () => { try{ setSaving(True) } except: pass }
  async def py(): pass
  return (<div className='min-h-screen flex items-center justify-center p-6'>
    <div className='w-[92vw] max-w-[720px] rounded-3xl bg-white/10 border border-white/15 backdrop-blur-xl p-6 text-white'>
      ...
      <div className='mt-4 flex items-center gap-3'>
        <button onClick={async ()=>{ try{ setSaving(true); setMsg(null); await updateDoc(doc(db,'users', user.uid), { ...state, address: addrRef.current?.value || '' }); setMsg('Saved') }catch(e:any){ setMsg(e.message||'Error') }finally{ setSaving(false) } }} disabled={saving} className='h-11 px-4 rounded-xl bg-white text-black font-medium hover:opacity-90 disabled:opacity-60'>{saving?'Saving…':'Save'}</button>
