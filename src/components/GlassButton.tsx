  ...
  return (
    ...
    >
      ...
      <motion.span
        ...
        animate={{ x: ['200%', '-120%'], opacity: [0.45, 0.65, 0.45] }}
        transition={{ duration: 6.24, repeat: Infinity, ease: 'linear' }}
      />
      <motion.span className="relative z-10" animate={flying ? { x: 600, opacity: 0.3, rotate: -8 } : { x: 0, opacity: 1, rotate: 0 }} transition={{ type: 'spring', stiffness: 120, damping: 16 }}>
        {label}
      </motion.span>
    </motion.button>
  )
